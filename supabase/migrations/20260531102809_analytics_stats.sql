-- Dashboard analytics, aggregated on read (no maintained rollup table — data volume is tiny and
-- always-fresh matters more). Shop functions are member-gated (SECURITY DEFINER); the customer
-- one runs as the caller (RLS scopes to own orders).

-- One-row shop summary for the shop dashboard.
create or replace function public.shop_stats(p_shop_id uuid)
returns table (
  revenue_total numeric, revenue_today numeric,
...
create or replace function public.shop_revenue_daily(p_shop_id uuid)
...
returns table (orders_count bigint, total_spent numeric, total_saved numeric)
language sql security invoker set search_path = '' stable as $$
  select count(*), coalesce(sum(total), 0), coalesce(sum(discount), 0)
  from public.orders where customer_id = (select auth.uid());
$$;

revoke execute on function public.shop_stats(uuid)          from public, anon;
revoke execute on function public.shop_revenue_daily(uuid)  from public, anon;
...
grant  execute on function public.shop_revenue_daily(uuid)  to authenticated;
